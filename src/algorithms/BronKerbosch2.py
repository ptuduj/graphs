...
def rec_bron_kerboschl(R, P, X, graph):
    ...
    for v in P:
        ...
        out.extend(rec_bron_kerboschl(R.add(v), P.elems_from(v).intersect(nb), P.elems_to(v).add(v).union(X).intersect(nb), graph))
    return out
...
def spark_bron_kerbosh(v, R, P, X, graph):
    ...
    nb = graph.out_neighbours(v)
    return rec_bron_kerboschl(R.add(v), P.elems_from(v).intersect(nb), P.elems_to(v).add(v).union(X).intersect(nb), graph)


def map_bron_kerboschl(graph, x):
    v = x.vId
    nb = x.neighbours
...
def bron_kerboschl(sc, graph):
    ...
    return graph \
        .get_rows() \
        .flatMap(lambda x: map_bron_kerboschl(non_spark_graph.value, x))\
