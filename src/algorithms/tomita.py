...
def rec_tomita(R, P, X, graph):
    ...
    for v in P.difference(choose_pivot(P, X, graph)):
        ...
        out.extend(rec_tomita(R.add(v),P.intersect(nb),X.intersect(nb),graph))
        P = P.remove(v)
        X = X.add(v)
    return out


def spark_tomita(graph, x):
    v = x.vId
    nb = x.neighbours
    ...
    nb = x.neighbours
    return rec_tomita(R, P.intersect(nb), X.intersect(nb), graph)
...
def tomita(sc, graph):
    ...
    return graph \
        .get_rows() \
        .flatMap(lambda x: spark_tomita(non_spark_graph.value, x))\
        .collect()